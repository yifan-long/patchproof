...
from ..evidence.canonical import canonical_json, hash_json
...
from ..workspace.artifact_policy import copytree_without_oracles, sanitize_check_output, tree_identity
...
class EvaluationOrchestrator:
    ...
    async def _run_pair(
        ...
        with tempfile.TemporaryDirectory(prefix=f"patchproof-eval-{case.id}-{repeat}-") as directory:
            ...
            # Import the concrete benchmark adapter only where the real pair
            # is executed.  The orchestration layer owns pairing and gates;
            # benchmark owns the two execution strategies.  Keeping this
            # dependency local removes the benchmark/evaluation import cycle.
    ...
    async def _initial_failure_gate(
        ...
        shared_evidence = canonical_json(evidence)
        return {
